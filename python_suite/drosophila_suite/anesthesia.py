...
class AnesthesiaAnalyzer:
    ...
    def evaluate_states(
        ...
        """
        逐通道进行三阶段状态判定:
        - 基线期保留全部自然停歇和微动，直接对给药前全量速度取分位数作为 baseline_speed。
        - 深度麻醉严禁透支给药前静止，仅从 gas_onset 开始往后扫描。
        - 镇静判定优先识别摔落箱底并不再攀爬的行为，低位停留做兜底。
        """
        ...
        for cid, grp in cleaned_df.groupby("chamber_id"):
            ...
            if len(pre_gas_speeds) >= int(self.fps):
                ...
                if len(moving_speeds) >= int(self.fps):
                    baseline_speed = float(np.percentile(moving_speeds, self.baseline_quantile * 100))
                else:
            ...
            if first_sedate_idx is None and len(post_indices) >= bottom_quiescent_frames:
                ...
                quiescent_at_bottom = (
                    ...
                    & (rolling_speed <= max(self.anesthesia_speed_thresh, baseline_speed * 0.25))
                )
                rolling_quiescent = (
